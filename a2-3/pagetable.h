...
typedef struct { 
  ...
  //int ref_list[PTRS_PER_PGDIR*PTRS_PER_PGTBL]; // A list of what lines in the tracefile this 
  int next_ref;
                             // entry is referenced on.
  int cur_pos;
} pgtbl_entry_t;    
...
struct frame {
	...
	pgtbl_entry_t *pte;// Pointer back to pagetable entry (pte) for page
        ...
        time_t time_in; // Time that the frame was loaded into phycial memory.
        time_t last_used; // Time that the frame was most recently referenced.
};
