...
def TrainNN(num_hiddens, eps, momentum, num_epochs):
  ...
  for epoch in xrange(num_epochs):
    ...
    train_CE = np.sum(abs(prediction - target_train) > 0.5) / float(prediction.size)
    ...
    valid_CE = evaluate2(inputs_valid, target_valid, W1, W2, b1, b2)
  ...
  final_train_error = evaluate2(inputs_train, target_train, W1, W2, b1, b2)
  final_valid_error = evaluate2(inputs_valid, target_valid, W1, W2, b1, b2)
  final_test_error = evaluate2(inputs_test, target_test, W1, W2, b1, b2)  
...
def evaluate2(inputs, target, W1, W2, b1, b2):
  """Evaluates the model on inputs and target."""
  h_input = np.dot(W1.T, inputs) + b1  # Input to hidden layer.
  h_output = 1 / (1 + np.exp(-h_input))  # Output of hidden layer.
  logit = np.dot(W2.T, h_output) + b2  # Input to output layer.
  prediction = 1 / (1 + np.exp(-logit))  # Output prediction.
  #CE = -np.mean(target * np.log(prediction) + (1 - target) * np.log(1 - prediction))
  cle = np.sum(abs(prediction - target) > 0.5) / float(prediction.size)
  return cle
