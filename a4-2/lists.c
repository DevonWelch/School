...
char * list_calendars(Calendar *cal_list) {
    ...
    int buf = 0;
    while (current != NULL) {
      buf += strlen(current->name) + 2;
      current = current->next;
    }
    buf += 1;
    char * char_cal_list = malloc(buf*(sizeof(char)));
    char_cal_list[0] = '\0';
    current = cal_list;
    while (current != NULL) {
      strncat(char_cal_list,current->name, (strlen(current->name))*sizeof(char));
      current = current->next;
      strncat(char_cal_list, "\r\n", 2*sizeof(char));
    }
    return char_cal_list;
}
...
char * list_users(User *user_list) {
    ...
    buf += 1;
    //This malloc is guaranteed to allocate the right
    //amount of memory because buf is equal to the
    //total length of all user name, plus 2 for each
    //username (to accommodate the /r/n added after every
    //one), plus one, for the null terminator.
    char * char_user_list = malloc(buf*(sizeof(char)));
    char_user_list[0] = '\0';
    current = user_list;
    while (current != NULL) {
      strncat(char_user_list, current->name, (strlen(current->name))*sizeof(char));
      current = current->next;
      strncat(char_user_list, "\r\n", 2*sizeof(char));
    }
    return char_user_list;
}
...
char * list_events(Calendar *cal_list, char *cal_name) {
    ...
    Calendar * this_cal;
    if ((this_cal = find_calendar(cal_list, cal_name)) == NULL) {
      return "Calendar DNE\0";
    }
    Event *cur = this_cal->events;
    int buf = 0;
    while (cur != NULL) {
      buf += strlen(cur->description) + strlen(ctime(&(cur->time))) + 4;
        cur = cur->next;
    }
    buf += 1;
    char * char_event_list = malloc(buf*(sizeof(char)));
    char_event_list[0] = '\0';
    cur = this_cal->events;
    while (cur != NULL) {
      strncat(char_event_list, cur->description, (strlen(cur->description))*sizeof(char));
      strncat(char_event_list, ": ", 2*sizeof(char));
      strncat(char_event_list, ctime(&(cur->time)), ((strlen(ctime(&(cur->time))))-1)*sizeof(char));
      cur = cur->next;
      strncat(char_event_list, "\r\n", 2*sizeof(char));
    }
    return char_event_list;
}
