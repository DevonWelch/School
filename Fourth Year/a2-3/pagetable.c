	...
	if(frame == -1) { // Didn't find a free page.
		...
		if (coremap[frame].pte->frame & PG_DIRTY) {
		  ...
		  if (coremap[frame].pte->frame & PG_ONSWAP) {
		    ...
		    swap_off = swap_pageout(frame, INVALID_SWAP);
...
pgtbl_entry_t *find_p(addr_t vaddr, char type) {
	...
	pgtbl_entry_t * cur_pgtbl = (pgtbl_entry_t *) (pgdir[idx].pde);
...
char *find_physpage(addr_t vaddr, char type) {
	...
	if (p->frame & PG_VALID) {
	  ...
	  miss_count += 1;
	  int fra = allocate_frame(p);
	  swap_pagein(fra, p->swap_off);
	  p->frame = fra << PAGE_SHIFT;
	  coremap[p->frame >> PAGE_SHIFT].time_in = 0;

	} else {

	  miss_count += 1;
	  int fra = allocate_frame(p);
	  init_frame(fra, vaddr);
	  p->frame = fra << PAGE_SHIFT;
	...
	coremap[p->frame >> PAGE_SHIFT].pte = p;
